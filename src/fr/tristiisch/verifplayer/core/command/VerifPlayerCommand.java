...
public class VerifPlayerCommand implements CommandExecutor {
	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String arg, final String[] args) {
		...
		if(sender instanceof Player) {
			...
			if(!Permission.ADMIN.hasPermission(sender)) {
				if(args.length == 0 && args[0].equalsIgnoreCase("about")) {
					sender.sendMessage(SpigotUtils.color("&2VerifPlayer &aV" + Main.getInstance().getDescription().getVersion() + " develop by Tristiisch."));
					return true;
				}
		...
		if(args.length == 0) {
			sender.sendMessage(SpigotUtils.color("&cUsage &7» &c/verifplayer <reload|about>"));

		} else if(args[0].equalsIgnoreCase("reload")) {
			final double time = CustomConfig.reloadConfigs();
			sender.sendMessage(SpigotUtils.color("&2VerifPlayer &7» &aThe config has been reload successfully, it took " + String.valueOf(time) + " seconds."));
