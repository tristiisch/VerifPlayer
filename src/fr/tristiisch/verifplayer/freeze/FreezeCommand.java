	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String arg, final String[] args) {
		...
		if(args.length == 1) {
			...
			if(Freeze.isFreeze(target)) {
				...
				player.sendMessage(Utils.color("&cVous avez unfreeze &4" + target.getName() + "&c."));
			} else {
				Freeze.freeze(target);
				player.sendMessage(Utils.color("&aVous avez freeze &2" + target.getName() + "&a."));
			}
			...
			if(Freeze.isFreeze(target)) {
				...
				player.sendMessage(Utils.color("&7Vous avez &cfreeze &8" + target.getName() + "&7 avec comme motif &c" + reason + "."));
			}
		} else {
