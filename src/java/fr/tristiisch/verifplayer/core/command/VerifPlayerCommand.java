...
public class VerifPlayerCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String arg, String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (!Permission.ADMIN_COMMAND_RELOAD.hasPermission(sender)) {
				...
				if (args.length == 0) {
					...
					sender.sendMessage(SpigotUtils.color("&2VerifPlayer &aV" + VerifPlayerPlugin.getInstance().getDescription().getVersion() + " develop by Tristiisch."));
					return true;
				}
		...
		if (args.length == 0) {
			...
			double time = CustomConfigs.loadConfigs();
